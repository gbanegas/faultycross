...
The adversary can trigger signing on the victim device, inject a clock glitch
at a chosen offset, and read the resulting signature. It does \emph{not} need
the faulted signature to be valid. This point is easy to miss and it matters
for Section~\ref{sec:counter}: the response $\resp_i = (\mu_i,\nu_i)$ of every
hidden round is present in the output regardless of the fault, so as soon as
the corrupted path discloses $\sigma_i$ for one such round, the extraction
of~\eqref{eq:meds-extract} succeeds --- even if the digest check would reject
the signature. What the adversary needs is not a valid signature but an
\emph{emitted} one.
...
We state the limits of the setup plainly. The target board is flashed
exclusively with the \texttt{stree\_to\_path\_to\_stree} function, with fixed
values for the reference tree, salt and challenge; we do not run a full MEDS
signature on the device. The consequence is that our measurements characterise
the reproducibility and the parameter window of each fault, but not the
difficulty of locating the corresponding instruction within a complete signing
trace, which on a real target would additionally require the attacker to
synchronise on the end of the commitment loop. We regard this as an
engineering obstacle rather than a structural one --- the target instructions
sit in a short, data-independent function executed once per signature --- but
it is not something these experiments establish.
...
By glitching the guarded increment \highlight{rhl}{\texttt{id++}} at line~12
