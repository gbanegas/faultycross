...
Let $\calT$ be the MEDS seed tree with $\lceil \log_2 t \rceil$ levels.
...
\]
By Claim~\ref{claim:meds-retain}, this equals
...
\begin{proof}[Extension to incomplete trees (Remark~\ref{rem:incomplete})]
When $t$ is not a power of~$2$, the MEDS tree is incomplete: some leaves
appear at depth $\lceil \log_2 t \rceil - 1$ rather than
$\lceil \log_2 t \rceil$.  The index-tracking arrays ensure that
$\SeedTreeToPath$ correctly identifies which nodes are leaves, assigns flags
according to~\eqref{eq:meds-leaf-flag}, and performs the bottom-up OR
pass~\eqref{eq:meds-or-prop} over the actual tree structure.

The proofs of both invariants above are purely graph-theoretic and depend
only on properties~\eqref{eq:meds-leaf-flag}--\eqref{eq:meds-flag-char},
which hold regardless of whether leaves appear at a uniform depth.  Hence
both invariants hold for any incomplete tree shape produced by MEDS.
\end{proof}
\begin{comment}
% ------------------------------------------------------------
