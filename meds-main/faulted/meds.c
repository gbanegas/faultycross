...
static uint8_t delta[MEDS_sec_seed_bytes] = {0x42,0x42,0x42,0x42,0x42,0x42,0x42,0x42,0x42,0x42,0x42,0x42,0x42,0x42,0x42,0x42,0x42,0x42,0x42,0x42,0x42,0x42,0x42,0x42,0x42,0x42,0x42,0x42,0x42,0x42,0x42,0x42};
  ...
  )
{
  ...
  // initialize delta with random bytes
  /* uint8_t delta[MEDS_sec_seed_bytes];
  ...
  printf("Reference tree : (%d bit seeds) \n", MEDS_st_seed_bytes);

  print_array(stree, SEED_TREE_size);
  ...
  printf("path = [");
  for (int i = 0; i < MEDS_t; i++){
    printf("0x%x, ", path[i*MEDS_st_seed_bytes]);
  }
  printf("]\n");
  ...
  printf("salta = [");
  for (int i = 0; i < MEDS_st_salt_bytes; i++){
    printf("0x%x, ", alpha[i]);
  }
  printf("]\n");

  print_array(path, MEDS_w);
...
/*   printf("A_inv = \n");
  pmod_mat_print(A_inv[x],MEDS_m, MEDS_m);
  printf("B_inv = \n");
  pmod_mat_print(B_inv[x],MEDS_m, MEDS_m);
 */ 
  // Create path from tree and challenge h
  stree_to_path_faulted(stree, h, path, alpha, x);

  printf("faulted path = [ ");
  for (int i = 0; i < MEDS_w; i++){
    printf("%x ", (uint8_t) path[MEDS_st_seed_bytes*i]);
  }
  printf("]\n");
  ...
  )
{
  ...
  path_to_stree_faulted(stree, h, path, alpha, x);
