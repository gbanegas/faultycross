...
int main(int argc, char *argv[])
{
  ...
  for (int round = 0; round < rounds; round++)
  {
    ...
    uint8_t sig_prime[CRYPTO_BYTES + sizeof(msg)] = {0};
    unsigned long long sig_len_prime = sizeof(sig);
    ...
    crypto_sign_faulted(sig_prime, &sig_len_prime, (const unsigned char *)msg, sizeof(msg), sk, -1);

    recover(msg_out, &msg_out_len, sig_prime, sizeof(sig_prime), pk, -1);
