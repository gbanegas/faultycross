...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{content/images/glitch_2d_results_ext2.png}
...
Crucially, the paths of many of these valid glitches contain a large set of intermediate nodes which provide sufficient 
coverage to compute all of the leaf values.

\subsubsection{Single hidden leaf recovery}

\paragraph{Compilation.} Higher compiler optimization levels made it difficult to precisely target the flags. 
Specifically, the \texttt{-O1}, \texttt{-O2}, \texttt{-O3}, and \texttt{-Os} optimization flags resulted in compiled 
...
level strictly on the targeted function. Applying \texttt{-O0} to the entire project was unfeasible, as it increased 
the binary size beyond the storage capacity of the target board. 

The disassembled code shows that the assignement is done in two instructions, first The CPU loads the immediate integer value 1 directly into 
the register \texttt{r3}, then it takes that value 1 from \texttt{r3} and writes it out to memory. 
...
According to ARM documentation, each of these instruction is 1 clock cycle long. 
We set the \texttt{repeat} parameter of the glitch controller to 2. 

\begin{figure}[htbp]
    \centering
        \includegraphics[width=\textwidth]{content/images/glitch_2d_results_ext3.png} 
...
\paragraph{Results.} The experiments confirm that the attack is reproducible. 
