...
\paragraph{Full tree recovery.}

This attack is successful if the returned path contains a single node which is the root node.

\textbf{Compilation.} This attack is carried out on a code compiled with the default \texttt{-Os} optimization.

\textbf{Trigger position.} We directly target the evaluation of the conditional and the branching instruction corresponding to line 19 of \ref{lst:path}). The compiled result is in \ref{lst:f1asm}. We fault for 3 clock cycles.
...
\paragraph{Subtree recovery.} This attack is successful if 
...
\paragraph{Parameters.} The evaluation of the condition and 
