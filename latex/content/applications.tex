...
\subsubsection{Model 2: Subtree Recovery}
...
Next, we establish a precise faulting window encompassing one or more target 
assembly instructions. To synchronize the injection hardware, we instrument the 
firmware by inserting a \texttt{trigger\_high()} 
instruction immediately before the target instruction block to signal the 
ChipWhisperer to begin 
counting clock cycles, followed by a \texttt{trigger\_low()} instruction to 
terminate the window. 
We then verify the disassembled binary to ensure that compiler optimizations 
have not rearranged, omitted, or reordered these trigger placements.
...
We iteratively sweep three spatial and temporal 
parameters to map the physical fault space and identify 
high-yield vulnerability regions. The parameter 
\texttt{width} controls the width of the glitch pulse, 
obtained by XORing the system clock, and is expressed as a 
percentage of the clock period ranging from 0\% to 50\%. 
The parameter \texttt{offset} specifies the phase shift 
between the glitch pulse and the system clock and is varied 
from -50\% to 50\% of the clock period. Finally, 
\texttt{ext\_offset} denotes the number of clock cycles 
elapsed between the trigger signal and the start of the 
glitch injection.
...
\textit{Model 1: Full tree recovery.}
This attack is successful if the returned path contains a 
single node which is the root node.

\textbf{Compilation.} This attack is carried out on a 
code compiled with the default \texttt{-Os} optimization.

\textbf{Trigger position.} We directly target the 
evaluation of the conditional and the branching 
instruction corresponding to line 19 of \ref{lst:path}). 
The compiled result is in \ref{lst:f1asm}. We fault for 3 
clock cycles.
...
\textbf{Parameters.} Figure~\ref{fig:fault1_results} shows that the attack has
...
\begin{figure}[htp]
    \centering
    \includegraphics[width=0.85\textwidth]{content/images/glitch_3d_results.png} 
...
\textit{Model 2: Subtree recovery.} This attack is successful if 
the returned path contains a hidden subtree, that is, we hit a node 
that gives us a subtree that should be hidden.


\textbf{Compilation.} This is carried out on a code compiled with the default \texttt{-Os} optimization.

\textbf{Trigger position.} We set the trigger to frame lines $19$ of Listing~\ref{lst:path}. The compiled result is in Listing~\ref{lst:f2asm}.
...
\textbf{Parameters.} The evaluation of the condition and 
the incrementation of \texttt{id} require 3 clock cycles. 
We therefore set \texttt{repeat} to $3$. We consider 
the attack successful if we obtain the faulted path pattern 
...
\begin{figure}[htp]
    \centering
    \includegraphics[width=0.8\textwidth]{content/images/glitch_2d_results_ext2.png}
...
\paragraph{Model 3: Single hidden leaf recovery.}
In this model, we recover only a hidden leaf. 

\textbf{Compilation.} Higher compiler optimization levels made it difficult to precisely target the flags. 
...
\textbf{Trigger position.} For the unoptimized version, we position the glitch high trigger right before the \texttt{index\_leaf} assignment and the low trigger right after. This results in the compiled code in listing~\ref{lst:f3asm}. 
...
\paragraph{Parameters.} The disassembled code shows that the assignement is done in two instructions, first The CPU loads the integer value 1 into 
...
\begin{figure}[htbp]
    \centering
        \includegraphics[scale=0.3]{content/images/glitch_2d_results_ext3.png} 
