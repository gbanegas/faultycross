...
\begin{enumerate}[leftmargin=*]
  \item $\Setup(1^{\lambda}) \to (\seed_r,\salt)$:
        samples a master seed $\seed_r \getsr \{0,1\}^{\lambda}$
        and a public salt.

  \item $\Expand(\seed_r,\salt)
        \to (\calT,(\eseed_0,\ldots,\eseed_{t-1}))$:
        builds a binary tree $\calT$ by top-down
        \PRG-expansion from $\seed_r$. Each leaf
        $i\in\{0,\ldots,t-1\}$ holds a seed $\eseed_i$.
        Tree topology may be complete or incomplete.

  \item $\Chal(\cmt,\msg)
        \to \mathbf{c}=(c_0,\ldots,c_{t-1})$:
        derives a Fiat--Shamir challenge from the
        commitment and message.

  \item $\Decide(\mathbf{c})
        \to (\calH,\calR)$:
        partitions $\{0,\ldots,t-1\}$ into the hidden
        set $\calH=\{i:c_i\neq0\}$ and the revealed
        set $\calR=\{i:c_i=0\}$.

  \item $\Publish(\calT,\calH)\to\tpath$:
        returns the minimal set of nodes from which
        every $\eseed_i$ with $i\in\calR$ is
        reconstructible while no $\eseed_i$ with
        $i\in\calH$ is derivable.

  \item $\Recon(\tpath,\mathbf{c},\salt)
        \to \{\eseed_i:i\in\calR\}$:
        verifier-side reconstruction of revealed
        leaf seeds from $\tpath$.
\end{enumerate}
\end{multicols}
\end{definition}
...
a power of two, exactly as in LESS-v2). $\Chal$ expands each leaf into ephemeral
...
\begin{definition}[Faulted Signing Oracle]
\label{def:fault-oracle}
Let $\Pi=(\Setup,\Expand,\Chal,\Decide,\Publish,\Recon)$ be a \GZKST
instance and let $(\sk,\pk)$ be a fixed key pair. For a node
$v^*\in V(\calT)$, the oracle $\Oracle^{v^*}(\sk,\pk)$ signs a
message $\msg$ as usual except that, during
