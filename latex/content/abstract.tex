...
of the \GGM-tree mechanism across the schemes discussed in
...
\begin{remark}
  In this instantiation of the $\Publish$ algorithm, 
  we rely on the path construction as described 
  in the MEDS submission~\cite{MEDS_PQC}. However, 
  the reference implementation uses a different algorithm, 
  that is further explained in Section~\ref{sec:app}.
\end{remark}


Using the same idea as in MEDS, we can apply 
to LESS-v2, and CROSS. One can easily see 
that the abstract model applies to LESS and CROSS. 
Also, we can see that we can derive Proposition~\ref{prop:less-gzkst} and 
Proposition~\ref{prop:cross-gzkst}. The proofs follow the same 
idea as in MEDS (in Appendix~\ref{app:proof-meds}). 
