...
$\Pi = (\Setup,\allowbreak\ \Expand, \Chal,
\Decide, \Publish, \Recon)$
parametrised by:
\begin{itemize}
...
(see Appendix~\hl{XXXXX}). Figure~\ref{fig:less-trees} shows an example 
of the expanded GGM tree as well as the hidden subtree, published nodes 
and published path.
\end{remark}
...
$\calH = \mathrm{supp}(\mathbf{b})$, $\calR = \{0,\ldots,t-1\} \setminus \calH$.
    \item $\Publish(\calT, \calH) \to \tpath$:
LESS-v2 builds an auxiliary flag tree of the same shape via three phases:
(i)~\emph{initialisation}: every node receives $\calF = 1$ except the root;
(ii)~\emph{leaf labelling} ($\LabelLeaves$):
$\calF(\mathrm{leaf}_i) = \mathbf{1}[i \in \calH]$;
(iii)~\emph{upward propagation} ($\ComputeSeeds$): $\calF(v) = \calF(\ell) \vee \calF(r)$.
The path $\tpath = \{v : \calF(v) = 0,\; \calF(\mathrm{parent}(v)) = 1\}$ is
the minimal publishable boundary (Figure~\ref{fig:less-trees}b).
    \item $\Recon(\tpath, \mathbf{c}, \salt) \to \{\eseed_i : i \in \calR\}$:
...
\begin{itemize}
  \item \textbf{MEDS}.
        For a hidden round $i$ with $h_i = j$, the response is
...
on every invocation of $\Oracle^{v^*}$.
Algorithm~\ref{alg:key-recovery} terminates and outputs $\widehat{\sk} = \sk$
with probability at least $1 - \delta$ after at most
$\lceil (s-1)/E_{\mathrm{dist}} \rceil \cdot \lceil \log(1/\delta) \rceil$
effective oracle queries.
\end{proposition}
...
\emph{distinct} secrets per effective query, where
$\bar{w} = w \cdot \ell / t$.  Then $\mathbb{E}[Q] \approx (s-1)/E_{\mathrm{dist}}$;
see Appendix~\hl{XXXX} for the derivation.
\end{proposition}

For all real MEDS parameter sets (Table~\ref{tab:scheme-params}), faulting
the root ($v^* = $ root, $\ell = t$) gives $\bar{w} = w$ and
$E_{\mathrm{dist}} \approx s - 1$, so $\mathbb{E}[Q] \approx 1$: a
\emph{single} effective faulted query suffices with high probability,
confirmed by the simulation in \S\ref{sec:app}.
